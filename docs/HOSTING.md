...
## Which host? (they are not equivalent — read this first)

This site reads live chain data and keeps all-time totals. Building those totals
means scanning a lot of block history, so **how well a host lets the app keep that
work between requests decides how fast the site feels**:

| host | cold-start behaviour |
|---|---|
| **Netlify** | **Fastest, zero tuning.** The persistent store (Blobs) and the scheduled cache-warmer both ship in this repo, so history is scanned once and visitors land on warm data. |
| **Vercel** | Fast. No Blobs, so a cold instance rebuilds its accumulators, but the edge honours the responses' `s-maxage`, so visitors share one scan. |
| **Cloudflare Workers** | Works, and fine for a hobby deploy — but Workers have no persistent store and end all work when the response is sent, so a cold isolate rebuilds from chain history and **the first visitor in a region can wait ~10s**. Later visitors hit the colo cache. Use a custom domain (Cloudflare does not cache on `*.workers.dev`). |
| **Self-host** | One long-lived process, so it scans once at boot and stays warm — as fast as Netlify. |

If you want it consistently snappy with no work, deploy to **Netlify**.

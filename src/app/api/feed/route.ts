...
// A Cloudflare Worker's own response is NOT stored by the CDN the way an
// origin's is (Netlify and Vercel both honour s-maxage at their edge for free),
// so on Workers the colo cache has to be read and written explicitly. Without
// it every cold isolate repeats the whole chain scan and only the visitor who
// warmed that one isolate sees a fast page. `caches` is a Workers global —
// undefined on Node, where this whole path is skipped and the Cache-Control
// header does the job instead.
...
export async function GET(req: NextRequest) {
  ...
  const edge = debug ? undefined : edgeCache();
  ...
  try {
    ...
    // ── Let the CDN hold this response ──────────────────────────────────────
    // The chain scan is expensive and the payload is identical for every
    // visitor, so a shared edge cache is the difference between "fast for
    // everyone" and "every cold instance re-scans": on hosts without a
    // persistent store (Cloudflare Workers has no Netlify Blobs, and its
    // in-isolate memory dies with the isolate) the CDN becomes the shared
    // cache. stale-while-revalidate is the important half — visitors get an
    // instant slightly-stale frame while one background request refreshes it,
    // so nobody waits on the scan.
    //
    // ONLY a COMPLETE payload is cacheable. Caching a degraded one (cold-start
    // empty events or null stats) would pin the empty state at the edge for
    // every visitor in that region — turning a one-request miss into a
    // site-wide outage. Those stay no-store so the next request retries.
    const complete = stats != null && snap.events.length > 0;
    const res = NextResponse.json(body, {
      headers: {
        "Cache-Control": complete
          ? `public, s-maxage=${Math.round(LIVE_POLL_MS / 1000)}, stale-while-revalidate=300`
          : "no-store",
      },
    });
    if (edge && complete) await edge.put(cacheKey, res.clone()).catch(() => {});
    return res;
  } catch (err) {

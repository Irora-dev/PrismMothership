...
import { colorSources, setExtractedTokenColor, type TokenChain } from "./token-visual";

// ─────────────────────────────────────────────────────────────────────────────
// Dominant-color extraction from a token's logo — the Spectrum operator system,
// ported (2026-08-03, the designer's bento color-pop ask). Unknown tokens otherwise
// get an address-hashed hue that can clash with the visible logo. The
// extracted color feeds token-visual's EXTRACTED cache, which tokenVisual
// reads below the curated/baked layers — brand colors stay authoritative.
//
...
function attempt(address: string, src: string, onFail: () => void): void {
  ...
  img.onload = () => {
    try {
      ...
      // Clamp into a band that reads as a tile fill on the dark surface.
...
function extract(address: string, chain: TokenChain, srcIdx = 0): void {
  const key = address.toLowerCase();
  if (srcIdx === 0) {
    if (attempted.has(key) || typeof document === "undefined") return;
    attempted.add(key);
  }
  const srcs = colorSources(address, chain);
  if (srcIdx < srcs.length) {
    attempt(address, srcs[srcIdx], () => extract(address, chain, srcIdx + 1));
  }
}
 ...
 *  (tokenVisual then returns the extracted color automatically). */
export function useTokenColors(assets: { address: string }[], chain: TokenChain): number {
  const v = useSyncExternalStore(subscribe, () => version, () => 0);
  useEffect(() => {
    for (const a of assets) extract(a.address, chain);
  }, [assets, chain]);
  return v;
}

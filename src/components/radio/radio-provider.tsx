...
export interface Track {
  ...
  /** streamed from another Mothership instance (cross-origin src) */
  remote?: boolean;
}
...
export function RadioProvider({ children }: { children: ReactNode }) {
  ...
  const ensureGraph = useCallback(() => {
    ...
    if (!a) return;
    // Remote tracks (another instance's radio, cross-origin, no CORS handshake)
    // must NOT enter the graph: a tainted MediaElementSource outputs silence,
    // and the one-shot connection would mute local tracks played later too.
    // Skipping keeps playback plain; visuals fall back to synthetic motion.
    if (tracksRef.current.some((t) => t.remote)) return;
    try {
  ...
  return (
    ...
    >
      ...
      <audio
        ref={audioRef}
        preload="none"
        // crossOrigin makes the browser demand CORS headers on the audio file and
        // REFUSE to play it without them. Local files are same-origin so it costs
        // nothing and buys the analyser (the reactive visuals). A remote station's
        // files are plain public MP3s on another host with no CORS headers, so the
        // attribute must come off or nothing plays at all — the analyser is already
        // skipped for remote tracks, and visuals fall back to synthetic motion.
        crossOrigin={tracks.some((t) => t.remote) ? undefined : "anonymous"}
        onPlaying={() => {
          setPlaying(true);
          setLoading(false);
        }}

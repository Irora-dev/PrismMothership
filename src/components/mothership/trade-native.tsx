...
export function TradeNativePanel() {
  ...
  // one dismissal for the post-trade popup, so the backdrop, Esc and the Done
  // button can never disagree about how to get out of it
  ...
  return (
    ...
    <main className="relative z-10 mx-auto w-full max-w-[1536px] space-y-6 p-4 sm:p-6">
      ...
      {phase === "done" && result && (
        // Every exit works, and the card can never trap its own buttons. This
        // overlay used to be dismissible ONLY via the Done button, sat BELOW the
        // authorize sheet's layer, and centred a tall card inside a non-scrolling
        // `fixed inset-0` box — so on a short viewport the buttons ended up off
        // screen behind a full-page blur with no way out but a refresh (observed
        // after a real trade). Now the backdrop closes it, Esc closes it, and the
        // overlay scrolls.
        <div
        ...
        >
          <div
            ...
            className="relative my-auto w-full max-w-md overflow-hidden rounded-2xl p-8 text-center"

...
- **Cloudflare:** Workers via OpenNext. Easiest: send the user the **Deploy to
  Cloudflare button** (top of README) — it copies the repo to their GitHub, wires
  build-on-push, prompts for the env vars, deploys. CLI alternative: `npm run deploy`
  (one command — `wrangler.jsonc` carries the build), headless with
  `CLOUDFLARE_API_TOKEN` set; env vars become Worker secrets. Never
